...
public class FloatstoneHead : MonoBehaviour {
	...
	void Update () {
		...
		if (Vector2.Distance (this.transform.position, destination) < 3.0f) {
			chooseNextDestination();
		}
		turnFraction+= Time.deltaTime;
		this.rigidbody2D.velocity = Vector2.Lerp(originalVector,targetVector,turnFraction) * 3.0f;
	...
	void chooseNextDestination(){
		...
		targetVector = destination.normalized;
		turnFraction = 0;
	}
}
